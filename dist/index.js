...
(async () => {
    ...
    const scrapeJobs = async (pageCount) => {
        if (pageCount > 1)
            return;
        console.log("Page count: ", pageCount);
        console.log(page.url());
        let popUpClose = await page.$("#mosaic-modal-mosaic-provider-desktopserp-jobalert-popup > div > div > div.icl-Modal > div > button");
        ...
        for (let i = 0; i < liElements.length; i++) {
            ...
            const title = (await jobPostEl
                ?.$eval("div.jobsearch-JobInfoHeader-title-container", (el) => el.textContent)
                .catch((err) => {
                errors.push(errToObj(err));
                console.log(err);
            }))?.replace(" - job post", "");
            // Get the company name
            const company = (await jobPostEl
            ...
            const companyInfo = (await jobPostEl
            ...
            const reviews = (companyInfo?.[0] && parseInt(companyInfo?.[0].replace(",", ""))) ||
                undefined;
            // Get the location
            const location = companyInfo?.[1];
            const salary = (await jobPostEl
                ?.$eval("#jobDetailsSection > div:nth-child(2) > div > span", (el) => el.textContent)
                .catch((err) => {
                errors.push(errToObj(err));
            })) || undefined;
            const jobType = (await jobPostEl
            ...
            const qualifications = (await jobPostEl
                ?.$$eval("#qualificationsSection > div.jobsearch-ReqAndQualSection-item--wrapper > div > ul > li", (el) => {
            ...
            // Get the benefits
            const benefits = (await jobPostEl
            ...
            const jobDescription = (await jobPostEl
            ...
            const jobInsights = (await jobPostEl
                ?.$$eval("#belowFullJobDescription + div > p, #mosaic-belowFullJobDescription + div > ul > li", (el) => {
                const insights = [];
                for (let i = 0; i < el.length; i++) {
                    const text = el[i].textContent;
        ...
        const nextPageButton = await page.$("#jobsearch-JapanPage > div > div > div.jobsearch-SerpMainContent > div.jobsearch-LeftPane > nav > div > a");
